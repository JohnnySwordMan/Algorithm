## 二叉树
+ [判断给定数组是否是二叉搜索树的后序遍历](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/VerifySquenceOfBST.java)  
+ [二叉搜索树与双向链表](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/TreeToListNode.java)  
+ [二叉树的深度](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/TreeDepth.java)
+ [根据前序、中序遍历重建二叉树](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/ReConstructBinaryTree.java)
+ [按之字型打印二叉树](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/PrintTreeByZhi.java)
+ [层次遍历二叉树](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/PrintFromTopToBottom.java)
+ [二叉树的镜像](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Mirror.java)
+ [层次遍历，按行的样式打印](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/LevelOrder.java)
+ [最低公共祖先](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/LCA.java)
+ [二叉搜索树的第k个节点](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/KthNode.java)
+ [平衡二叉树的判断](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/IsBalanced.java)
+ [二叉树中和为某一值的路径](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FindPath.java)

## 栈与队列
+ [两个栈实现队列](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/TwoStack.java)  

## 字符串
+ [左旋字符串](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Translation.java)  
+ [翻转单词顺序](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/ReverseSentence.java)  
+ [替换空格](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/ReplaceSpace.java)
+ [输入字符串，按字典序打印出所有排列](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Permutation.java)
+ [所有字符串的最长公共前缀](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/LongestCommonPrefix.java)  

## 链表
+ [反转链表](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/ReverseList.java)
+ [从尾到头打印链表](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/PrintListFromTailToHead.java)
+ [合并两个有序链表](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Merge.java)
+ [复杂链表的复制](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Clone.java)
+ [链表中倒数第k个节点](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FindKthToTail.java) 
+ [两个链表的第一个公共节点](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FindFirstCommonNode.java)
+ [链表中环的入口节点](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/EntryNodeOfLoop.java) 
+ [在O(1)时间内删除链表节点](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/DeleteNode.java)
+ [删除链表中重复的节点](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/DeleteDuplication.java)
+ [二叉搜索树与双向链表](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Convert.java)
+ [复杂链表的复制](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Clone.java)

## 数组
+ [调整数组顺序使奇数位于偶数之前](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/ReOrderArray.java)
+ [把数组排成最小的数](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/PrintMinNumber.java)
+ [数组中出现次数超过一半的数字](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/MoreThanHalfNum.java)
+ [旋转数组的最小数字](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/MinNumberInRotateArray.java)
+ [圆圈中最后剩下的数字、约瑟夫环](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/LastRemaining.java)
+ [数组的逆序对](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/InversePairs.java)
+ [查找第k个大小的元素](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/GetNumberOfK.java)
+ [最小的k个数](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/GetLeastNumbers_Solution.java)
+ [TopK](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/TopK.md)
+ [第一个只出现一次的字符](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FirstNotRepeatingChar.java)
+ [和为s的两个数字](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FindNumbersWithSum.java) 
+ [连续子数组的最大和](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FindGreatestSumOfSubArray.java)
+ [和为s的连续正数序列](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FindContinuousSequence.java)
+ [查找二维数组中是否存在target](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Find.java)
+ [数组中重复的数字](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/Duplicate.java)

## 排序  
+ [归并排序](https://github.com/JohnnySwordMan/algorithm/blob/master/Sort/%E5%BD%92%E5%B9%B6%E6%8E%92%E5%BA%8F.md)   

## 其他
+ [跳台阶](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/JumpFloor.java)
+ [变态跳台阶](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/JumpFloorII.java)
+ [数组中只出现一次的数字](https://github.com/JohnnySwordMan/algorithm/blob/master/JZOffer/FindNumsAppearOnce.java) 

## 动态规划
## 贪心算法
